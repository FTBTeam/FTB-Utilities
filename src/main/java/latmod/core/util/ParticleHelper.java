...
public class ParticleHelper
{
	...
	public static void spawnFallParticles(World w, double x, double y, double z, int c, Block b, int m) // RenderGlobal // 2006
	{
		...
		{
			double f3 = MathHelperLM.clamp(rand.nextFloat(), 0D, MathHelperLM.TWO_PI);
			double d5 = MathHelperLM.clamp(rand.nextFloat(), 0.75D, 1D);
			double d6 = 0.20000000298023224D + d3 / 100D;
			double d7 = (double)(MathHelperLM.cos(f3) * 0.2F) * d5 * d5 * (d3 + 0.2D);
			double d8 = (double)(MathHelperLM.sin(f3) * 0.2F) * d5 * d5 * (d3 + 0.2D);
	...
	{ if(LatCore.proxy.inGameHasFocus()) w.spawnParticle(s, x, y, z, a, b, c); }
