...
public class ClaimedAreasClient
{
	...
	public static ChunkType getTypeE(int dim, int x, int z, LMPlayerClient p)
	{ return ChunkType.getChunkTypeFromI(getType(dim, x, z), p); }
	...
	public static void getMessage(int dim, int x, int z, FastList<String> l, LMPlayerClient p, boolean shift)
	{
		int type = getType(dim, x, z);
		ChunkType typeE = ChunkType.getChunkTypeFromI(type, p);
		...
		if(typeE != null)
		{
			if(typeE.isClaimed())
			{
				...
				if(owner != null) l.add(typeE.chatColor + owner.getName());
			}
			else l.add(typeE.chatColor + typeE.getIDS());
		}
	}
}