...
public class GuidePage extends FinalIDObject implements IJsonObject // GuideFile
{
	...
	public void loadFromFiles(File f) throws Exception
	{
		...
		if(f.isDirectory())
		{
			...
			if(f1 != null && f1.length > 0)
			{
				...
				GuidePage c1 = getSub(f.getName());
				for(File f2 : f1) c1.loadFromFiles(f2);
			}
		}
		else if(f.isFile())
		{
			...
			{
				GuidePage c1 = getSub(LMFileUtils.getRawFileName(f));
	...
	{
		if(imageFolder != null) return imageFolder;
		return (parent == null) ? null : parent.getImageFolder();
	}
}