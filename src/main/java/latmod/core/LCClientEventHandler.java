...
public class LCClientEventHandler
{
	...
	public void preTexturesLoaded(TextureStitchEvent.Pre e)
	{
		if(e.map.getTextureType() == 0)
			LatCoreMCClient.blockNullIcon = e.map.registerIcon(LatCore.mod.assets + "nullIcon");
	}
	...
	{
		if(LatCoreClient.key.isPressed() && LatCore.proxy.inGameHasFocus())
		{
			EntityPlayer ep = LatCore.proxy.getClientPlayer();
